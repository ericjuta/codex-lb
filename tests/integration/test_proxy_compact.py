...
async def test_proxy_compact_repeated_401_settles_reservation_if_error_recording_fails(async_client, monkeypatch):
    ...
    settle_compact_usage = AsyncMock(return_value=(True, True))
...
@pytest.mark.asyncio
async def test_backend_compact_usage_settlement_failure_does_not_penalize_successful_account(async_client, monkeypatch):
    email = "compact-settlement-fail@example.com"
    raw_account_id = "acc_compact_settlement_fail"
    response = await async_client.post(
        "/api/accounts/import",
        files={"auth_json": ("auth.json", json.dumps(_make_auth_json(raw_account_id, email)), "application/json")},
    )
    ...
    monkeypatch.setattr(proxy_module.LoadBalancer, "record_error", record_error)

    response = await async_client.post(
        "/backend-api/codex/responses/compact",
        json={"model": "gpt-5.6-sol", "instructions": "hi", "input": []},
    )
