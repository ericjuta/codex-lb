...
def _websocket_account(account_id: str = "acct_ws_proxy") -> proxy_module.Account:
    return proxy_module.Account(
        id=account_id,
...
def _ws_function_call_events(*, output_index: int, item_id: str, call_id: str, name: str) -> list[dict[str, Any]]:
    return [
        {
            "type": "response.output_item.added",
            "output_index": output_index,
...
def test_backend_responses_websocket_folded_turn_aliases_previous_response_id(app_instance, monkeypatch):
    # A folded turn's client-visible response id (round 1) differs from the
    # final hidden round's stored upstream id. A follow-up turn chaining the
    # visible id must be forwarded upstream with the hidden round's id, or the
    # upstream rejects the function_call_output ("No tool call found for
    ...
    round_two = [
        {"type": "response.created", "response": {"id": "resp_ws_fold_h", "status": "in_progress", "output": []}},
        *_ws_reasoning_events(output_index=0, item_id="rs_2", encrypted_content="enc2"),
        *_ws_function_call_events(output_index=1, item_id="fc_1", call_id="call_folded", name="shell"),
        _ws_completed("resp_ws_fold_h", input_tokens=120, output_tokens=20, reasoning_tokens=10),
    ]
    turn_two = [
        {"type": "response.created", "response": {"id": "resp_ws_fold_t2", "status": "in_progress", "output": []}},
        *_ws_message_events(output_index=0, item_id="msg_t2", text="tool result received"),
        _ws_completed("resp_ws_fold_t2", input_tokens=50, output_tokens=10, reasoning_tokens=5),
    ]
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        return SimpleNamespace(id="acct_ws_fold_alias", codex_installation_id="account-installation"), fake_upstream
    ...
    second_turn_payload = {
        ...
        "instructions": "",
        "reasoning": {"effort": "high"},
        "previous_response_id": "resp_ws_fold_v",
        "input": [{"type": "function_call_output", "call_id": "call_folded", "output": "ok"}],
        "stream": True,
    }

    first_turn_events: list[dict[str, Any]] = []
    second_turn_events: list[dict[str, Any]] = []
    with TestClient(app_instance) as client:
        with client.websocket_connect(
            "/backend-api/codex/responses",
            headers={
                "Authorization": "Bearer external-token",
                "chatgpt-account-id": "external-account",
                "session_id": "thread-ws-fold-alias",
                "openai-beta": "responses_websockets=2026-02-06",
            },
        ) as websocket:
            websocket.send_text(json.dumps(first_turn_payload))
            while True:
                event = json.loads(websocket.receive_text())
                first_turn_events.append(event)
                if event.get("type") in {"response.completed", "response.failed", "response.incomplete", "error"}:
                    break
            websocket.send_text(json.dumps(second_turn_payload))
            while True:
                event = json.loads(websocket.receive_text())
    ...
    # The folded turn stays downstream-stable on the visible id and delivers
    # the hidden round's tool call.
    first_terminal = first_turn_events[-1]
    assert first_terminal["type"] == "response.completed"
    assert first_terminal["response"]["id"] == "resp_ws_fold_v"
    folded_call_ids = [
        event["item"]["call_id"]
...
def test_backend_responses_websocket_orphaned_tool_output_error_fails_closed(app_instance, monkeypatch):
    ...
    # must not reach the client; it is rewritten to the fail-closed
    # stream_incomplete terminal so the client retries with full history.
                ...
                text=json.dumps(
                    {
                        ...
                        "error": {
                            ...
                            "message": "No tool call found for function call output with call_id call_orphan.",
                            "param": "input",
                        },
                        "status": 400,
                    },
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        return _websocket_account("acct_ws_orphan"), fake_upstream
    ...
    request_payload = {
        ...
        "model": "gpt-5.4",
        "instructions": "",
        "previous_response_id": "resp_ws_prev_turn",
        "input": [{"type": "function_call_output", "call_id": "call_orphan", "output": "ok"}],
        "stream": True,
    }
    ...
    assert terminal_event["response"]["error"]["type"] == "server_error"
    assert "No tool call found" not in json.dumps(terminal_event)
    assert len(log_calls) == 1
...
def test_v1_responses_websocket_allows_native_tool_surface(app_instance, monkeypatch):
    upstream_messages = [
        _FakeUpstreamMessage(
            "text",
            text=json.dumps(
                {"type": "response.created", "response": {"id": "resp_v1_native_tools", "status": "in_progress"}},
                separators=(",", ":"),
            ),
        ),
        _FakeUpstreamMessage(
            "text",
            text=json.dumps(
                {"type": "response.completed", "response": {"id": "resp_v1_native_tools", "status": "completed"}},
                separators=(",", ":"),
            ),
        ),
    ]
    ...
    async def fake_connect_proxy_websocket(*args, **kwargs):
        del args, kwargs
        return SimpleNamespace(id="acct_v1_ws_native_tools"), fake_upstream
    ...
    sent_payloads = [json.loads(message) for message in fake_upstream.sent_text]
    assert sent_payloads[0]["tools"] == [{"type": "image_generation"}, {"type": "web_search"}]
...
def test_backend_responses_websocket_treats_typeless_upstream_error_as_terminal(app_instance, monkeypatch):
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        connect_attempts["count"] += 1
        return _websocket_account("acct_ws_proxy"), fake_upstream
...
def test_backend_responses_websocket_keeps_downstream_open_after_clean_upstream_close(app_instance, monkeypatch):
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        return _websocket_account(f"acct_ws_proxy_{len(connect_models)}"), upstreams[len(connect_models) - 1]
...
def test_backend_responses_websocket_emits_response_failed_before_close_on_upstream_eof(app_instance, monkeypatch):
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        return _websocket_account("acct_ws_proxy"), upstreams.pop(0)
...
def test_backend_responses_websocket_anchors_reasoning_led_incremental_delta(
    app_instance,
    monkeypatch,
):
    # The dominant incremental client shape: the delta begins with the prior
    # turn's reasoning/tool-call outputs. Upstream dedupes re-sent reasoning on
    # anchored turns, so the anchor must be injected and the delta sliced.
    def upstream_messages(response_id: str) -> list[_FakeUpstreamMessage]:
        return [
            _FakeUpstreamMessage(
                "text",
    ...
    first_upstream = _FakeUpstreamWebSocket(upstream_messages("resp_incr_anchor"))
    second_upstream = _FakeUpstreamWebSocket(upstream_messages("resp_incr_followup"))
    upstreams = deque([first_upstream, second_upstream])
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        return SimpleNamespace(id="acct_incr_anchor"), upstreams.popleft()
    ...
    followup_payload = {
        ...
        "model": "gpt-5.4",
        "instructions": "",
        "input": [first_input, *incremental_delta],
        "stream": True,
    }
    ...
    second_upstream_payload = json.loads(second_upstream.sent_text[0])
    assert second_upstream_payload["previous_response_id"] == "resp_incr_anchor"
    assert second_upstream_payload["input"] == incremental_delta
...
def test_backend_responses_websocket_recovers_created_only_sequenced_prewarm(
    ...
    if expect_recovery:
        ...
        assert log_calls[0]["request_id"] == "resp_ws_prewarm_first"
        assert log_calls[0]["status"] == "success"
