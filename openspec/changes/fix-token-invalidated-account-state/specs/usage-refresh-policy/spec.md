...
The system MUST treat an OAuth refresh failure as a permanent refresh-token or
session failure when it reports a credential-token error code such as
`token_expired`, `invalid_grant`, `refresh_token_expired`,
`refresh_token_reused`, or `refresh_token_invalidated`. The affected account MUST be
