...
- [x] 7.2 [P2] `update_model_for_request` now takes the fold-state lock and rewrites only rows at/above `max(folded_through, hourly_folded_through)` (round 2 corrected `min` → `max`: a row is un-folded by every rollup only above the most advanced watermark) — a client-reused request id can no longer mutate folded history unmirrored.
