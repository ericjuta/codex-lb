...
1. **Codex-internal events leak as the first SSE event.** The Codex backend emits a non-standard `codex.rate_limits` event, and the backend frequently (not always — it is throttled per rate-limit window) places it *before* `response.created`. `_normalize_public_stream_payload` has a catch-all `return payload, None` that passes through any event type it does not explicitly handle, so `codex.rate_limits` reaches the client verbatim. The OpenAI SDK's `_create_initial_response` raises `RuntimeError: Expected to have received 'response.created' before 'codex.rate_limits'` on the very first event — the stream never starts. Because the upstream throttles the event, the same request intermittently works or fails, which is the hardest failure mode to diagnose. Verified against a public codex-lb deployment: 4/5 streaming request shapes (plain text, tool call, structured output, error) fail; the 5th passed only because the rate-limit window had already been consumed.
